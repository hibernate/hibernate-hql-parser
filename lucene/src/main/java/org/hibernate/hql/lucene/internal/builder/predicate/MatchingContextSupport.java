...
/* package private */class MatchingContextSupport {
	...
	TermMatchingContext keyWordTermMatchingContext() {
		if ( fieldBridge != null ) {
			return ( (ConnectedTermMatchingContext) builder.keyword().onField( propertyName ) ).fieldBridge( fieldBridge ).ignoreAnalyzer();
		}
		else {
	...
	TermMatchingContext wildcardTermMatchingContext() {
		if ( fieldBridge != null ) {
			return ( (ConnectedTermMatchingContext) builder.keyword().wildcard().onField( propertyName ) ).fieldBridge( fieldBridge ).ignoreAnalyzer();
		}
		else {
	...
	RangeMatchingContext rangeMatchingContext() {
		if ( fieldBridge != null ) {
			return ( (ConnectedRangeMatchingContext) builder.range().onField( propertyName ) ).fieldBridge( fieldBridge ).ignoreAnalyzer();
		}
		else {
